...
import com.nhaarman.acorn.navigation.experimental.CompositeParallelNavigatorTest.TestDestination.Destination3
import com.nhaarman.acorn.navigation.experimental.CompositeParallelNavigatorTest.TestDestination.values
...
internal class CompositeParallelNavigatorTest {
    ...
    private val extraNavigator = spy(SavableTestScene(99))

    private var navigator1 = spy(SavableTestStackNavigator(listOf(navigator1Scene1)))
    private var navigator2 = spy(SavableTestStackNavigator(listOf(navigator2Scene1)))
    private var navigator3 = spy(SavableTestStackNavigator(listOf(navigator3Scene1)))
    ...
    inner class NavigatorStates {
        ...
        inner class ActiveNavigator {
            ...
            fun `onBackPressed for two scenes in first destination does notify scene`() {
                ...
                listener.inOrder {
                    verify().scene(navigator1Scene1)
                    verify().scene(navigator1Scene2, TransitionData.forwards)
                    verify().scene(navigator1Scene1, TransitionData.backwards)
                    verifyNoMoreInteractions()
                }
            }
        }
    ...
    @Nested
    inner class ChildInteractionOnBackPress {
        ...
        override fun createNavigator(destination: TestDestination, savedState: NavigatorState?): Navigator {
            if (savedState != null) {
                return when (destination) {
                    ...
                    Destination3 -> SavableTestSingleSceneNavigator(navigator3Scene1, savedState)
                }
            }
    ...
    private class NonBackPressableNavigator : Navigator {
