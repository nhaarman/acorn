...
internal class CircleCINavigatorProvider(
    private val circleCIModule: CircleCIModule
) : AbstractNavigatorProvider<Navigator>() {

    override fun createNavigator(savedState: NavigatorState?): Navigator {
        val recentBuildsService = circleCIModule.webServiceFactory.createRecentBuildsService()
        return CircleCINavigator(RecentBuildsProvider.create(recentBuildsService), savedState)
    }
}