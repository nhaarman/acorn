 ...
 *
 *  - [HelloMVVMDataBindingNavigator] manages the navigation flow, in this case showing
 *    a simple Scene.
 *  - [HelloMVVMDataBindingScene] presents a text to the UI when available.
 *  - [HelloMVVMDataBindingViewController] implements the view to show the text.
 */
class MainActivity : AcornActivity() {

    override fun provideNavigatorProvider(): NavigatorProvider {
        return HelloMVVMDataBindingNavigatorProvider
    }

    override fun provideViewControllerFactory(): ViewControllerFactory {
        return bindViews {
            bind(
                defaultKey<HelloMVVMDataBindingScene>(),
                R.layout.hello_mvvm,
                ::HelloMVVMDataBindingViewController
            )
        }
    }
}