...
class PlacesScene(
...
) : BasicScene<PlacesContainer>(savedState),
    SavableScene {

    override val key = PlacesScene.key
    ...
    override fun attach(v: PlacesContainer) {
        super.attach(v)
        listenerDisposable = v.setDestinationSelectedListener(listener)
    }

    override fun detach(v: PlacesContainer) {
    ...
    companion object {
        ...
        val key = SceneKey.defaultKey<PlacesScene>()
    }
}
